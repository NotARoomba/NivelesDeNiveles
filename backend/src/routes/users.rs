...
use std::sync::Arc;
use crate::{status::{ResponseBody, STATUS_CODES}, types::{DangerLevel, User}, utils::{create_configuration, create_notification, Collections}};
...
pub async fn update_user(extract::Json(u): extract::Json<User>, collections: &Collections) -> impl IntoResponse {
    ...
    if user.location.coordinates.len() == 2 {
        ...
        let current_incidents = incidents.by_ref().collect::<Vec<_>>().await.into_iter().filter(|i| {
            ...
            haversine::Units::Kilometers) < incident.range as f64
        }).collect::<Vec<_>>();
        let before_user = collections.users.find_one(doc! {"number": user.number.clone()}).await.unwrap().unwrap();
        let before_incidents = incidents.collect::<Vec<_>>().await.into_iter().filter(|i| {
            let incident = i.as_ref().unwrap();
            haversine::distance(
                 haversine::Location{
        ...
        if current_danger_level != before_danger_level {
            ...
            let res = onesignal_rust_api::apis::default_api::create_notification(&configuration, *notification).await;
            match res {
                Ok(_) => (),
    ...
    let res = collections.users.update_one(doc! {"number": user.number}, doc! {"location": {"coordinates" : [user.location.coordinates[0], user.location.coordinates[1]], "type": user.location.location_type}}).upsert(true).await.unwrap();
